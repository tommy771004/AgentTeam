...
**Busy policy.** Different conversation threads execute independently, up to the `maxConcurrentRuns` safety cap. Builtin Pi same-session follow-ups use Host-owned true steer or the Host FIFO queue, selected once from `settings.followUpMode` and then frozen on the submission. Answered and failed turns auto-drain the next queued follow-up; an interrupted turn pauses same-session queued work until the user explicitly starts it. External CLI cannot true-steer: the same default maps to the explicitly named「中止並接手」takeover; if its stopped process has not released capacity within the bounded wait, the renderer compatibility queue fallback preserves the new goal. Automation overflow and non-Host compatibility still use `agent/runQueue.ts` (FIFO + dedupe + persist, max 24). Capacity is held in the run registry, so `agentStore.isRunning` is derived rather than a sole lock.
...
Automation runs set `unattended: true`: HITL asks and safety interventions fail closed without waiting for a person. Attended task asks remain pending until the user answers or the run is cancelled.
