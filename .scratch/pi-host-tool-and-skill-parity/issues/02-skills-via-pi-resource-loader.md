...
- [x] Host 擁有一個技能目錄，一個技能一個 `SKILL.md`，frontmatter 帶 `name` / `description`
- [x] `DefaultResourceLoader` 透過 `additionalSkillPaths` 指向該目錄
- [x] 一個 turn 的系統提示出現 `<available_skills>`，內含技能的 name、description、location
- [x] 模型能循 catalog 給的 location 讀到技能 body
- [x] `PiResourceRegistry` 由 loader 的 `getSkills()` 填入，`resources/list` 回報實際找到的技能而非空陣列
- [x] 格式錯誤的技能被回報為 diagnostic，不是靜默略過
- [x] 測試在單一接縫：spawn `dist-electron/pi-host.js`，斷言 `resources/list` 與 turn 的系統提示
