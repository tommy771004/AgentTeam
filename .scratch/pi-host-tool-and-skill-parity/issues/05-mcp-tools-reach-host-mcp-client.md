...
- [ ] `mcp_list_tools` / `mcp_call` 註冊為 extension tools，走 Host 既有的 MCP 路徑
- [ ] 列出的工具與 `tools/list` 攤平出來的 MCP 工具一致，沒有第二份清單
- [ ] 未啟用或不存在的 extension id 回結構化錯誤
- [ ] 走同一套 Approval Decision（比照既有 `tools/mcp` 的核准處理）
- [ ] 測試在單一接縫，用既有的 MCP fixture
