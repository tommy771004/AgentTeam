...
- [ ] 每一對逐項證明 parity：parameter schema、成功結果形狀、錯誤結果形狀、streaming updates、取消、專案範圍、session recording
- [ ] 逃出 root 的路徑在新舊兩邊都同樣被拒絕（比照 `scripts/smoke-pi-equivalent-tools.mts`）
- [ ] parity 通過後刪除四個 renderer 工具與其註冊
- [ ] 目錄投影反映刪除結果，舊名稱不再出現
- [ ] 測試在單一接縫；parity 斷言與刪除在同一張票內完成
