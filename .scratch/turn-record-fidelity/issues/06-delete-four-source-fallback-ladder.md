...
- [ ] 執行過程記錄只從 Turn Record 推導，四層 fallback 梯子刪除（是刪除，不是重排優先序）
- [ ] live activity store 不再是任何持久化內容的輸入；其事件上限只約束記憶體快取
- [ ] 超過舊上限的長跑（>120 事件）在完成後仍可完整讀到執行過程
- [ ] 執行過程列的排序由 `seq` 決定
- [ ] Seam 2 smoke：以長帳本 fixture 斷言不因任何上限而遺失條目
- [ ] Seam 1 smoke：一次超過舊上限的回合，完成後記錄完整
