...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [08 Atomic settings persistence](release-qualification-hardening/issues/08-atomic-settings-persistence.md) | 01–07 已 resolved：candidate-only promotion 與完整 main-vault credential expand–contract 均有主鏈 smoke；Telegram／Webhook／custom-tool raw fields 已退出 renderer settings，legacy 只走 fail-closed migration ingress。下一步為 atomic settings persistence；Paid Beta 仍維持 NO-GO（0/43）。 |
