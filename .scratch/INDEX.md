...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [05 Telegram/Webhook vault migration](release-qualification-hardening/issues/05-telegram-webhook-vault-migration.md) | 01–04 已 resolved：candidate-only package、verified channel promotion 與 main-process credential expand contract 已有主鏈 smoke。Vault 支援 stable reference、metadata-only IPC、磁碟重啟與加密失效防護；下一步遷移 Telegram/Webhook legacy credentials。Paid Beta 仍維持 NO-GO（0/43）。 |
