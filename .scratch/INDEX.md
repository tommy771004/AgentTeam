...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [09 Deterministic qualification](release-qualification-hardening/issues/09-no-app-launch-deterministic-qualification.md) | 01–08 已 resolved：candidate-only promotion、main-vault credential expand–contract 與 atomic settings／last-good recovery 均有主鏈 smoke；failure matrix 證明 restart 只會得到完整舊／新 generation。下一步為不啟動 App 的 deterministic qualification；Paid Beta 仍維持 NO-GO（0/43）。 |
