...
|--------|------|----------|-------|
| **durable-memory-sqlite-migration** | [spec.md](durable-memory-sqlite-migration/spec.md) | [05 Builtin Pi scoped recall 與 Turn Record provenance](durable-memory-sqlite-migration/issues/05-builtin-recall-and-turn-record.md) · [07 Task run learning settlement](durable-memory-sqlite-migration/issues/07-run-learning-settlement.md) · [08 Learning／Settings UI Projection](durable-memory-sqlite-migration/issues/08-learning-settings-projection.md) · [13 Host storage lifecycle](durable-memory-sqlite-migration/issues/13-host-storage-lifecycle.md) | #01–04 resolved。#04 完成核准的 state 目錄 downgrade barrier、restart-safe JSON→SQLite cutover 與唯一 production authority；build、Pi gates、完整 smoke 全綠。[復原邊界](durable-memory-sqlite-migration/cutover-recovery.md)。05/07/08/13 現可並行；06 仍等 05。 |
