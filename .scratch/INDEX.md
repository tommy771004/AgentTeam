...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [02 Pre-qualification publication barrier](release-qualification-hardening/issues/02-pre-qualification-publication-barrier.md) | 01 已 resolved：packaged change evidence schema v2、真 UI change rows、release consumer 與 legacy migration 已對齊；下一步依 02 → 03 收口 Critical publication path。Paid Beta 仍維持 NO-GO（0/43），不得由本地 build 或 unsigned artifacts 推導 ready。 |
