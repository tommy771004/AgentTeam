...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [07 Legacy raw-secret contract removal](release-qualification-hardening/issues/07-remove-legacy-raw-secret-contract.md) | 01–06 已 resolved：custom-tool raw/encrypted migration、metadata UI、Pi Host pack 執行與 reflected-output redaction 已掛主鏈 owning smoke；下一步刪除 flat settings schema/default 的 migration compatibility 欄位。Paid Beta 仍維持 NO-GO（0/43）。 |
