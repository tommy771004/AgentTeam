...
|--------|------|----------|-------|
| **durable-memory-sqlite-migration** | [spec.md](durable-memory-sqlite-migration/spec.md) | [04 JSON → SQLite 原子遷移與 authority cutover](durable-memory-sqlite-migration/issues/04-atomic-json-sqlite-cutover.md) | #01–03 已 resolved。#04 已補 atomic migration core／source-hash marker／拒絕報告與 corrupt-state startup refusal，build 與 Pi qualification 綠；目前待補資訊：確認 Host state 檔案佈局變更以阻擋既有舊版覆寫。legacy JSON authority 尚未 cutover，後續 05–13 未解鎖。 |
