...
|--------|------|----------|-------|
| **durable-memory-sqlite-migration** | [spec.md](durable-memory-sqlite-migration/spec.md) | [03 Authority boundary 的 scope、policy 與 idempotency](durable-memory-sqlite-migration/issues/03-authority-policy-and-idempotency.md) | #01–02 已 resolved：共用 store contract、SQLite adapter、versioned Host protocol、post-commit revision event 與 restart durability 已掛主 smoke chain；legacy JSON authority 尚未 cutover。frontier 推進 #03，後續維持 expand 03–04 → 分支遷移 05–13 → failure matrix 14 → contract 15 → qualification 16。 |
