...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [10 Merge-base complexity qualification](release-qualification-hardening/issues/10-merge-base-complexity-qualification.md) | 01–09 已 resolved：共享 deterministic qualification 在 PR CI／release build 前 blocking 執行，包含完整 single ingress、Pi／collaboration ownership、security 與 settings durability；headless sentinel 覆蓋 shell、fork 與 Electron process 自身並證明不啟動 App。下一步把 complexity baseline 從 `HEAD^` 改為 PR merge-base；Paid Beta 仍維持 NO-GO（0/43）。 |
