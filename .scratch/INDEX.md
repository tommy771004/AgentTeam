...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [04 Credential vault expand contract](release-qualification-hardening/issues/04-credential-vault-expand-contract.md) | 01–03 已 resolved：candidate-only package、qualification-bound receipt 與唯一 protected publish owner 已收口；Beta/Stable channel、signed artifact identity、atomic promotion 與 fail-closed writer guard 均有主鏈 smoke。下一步擴充 main-process credential vault；Paid Beta 仍維持 NO-GO（0/43）。 |
