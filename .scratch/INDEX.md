...
| **turn-record-fidelity** | [spec.md](turn-record-fidelity/spec.md) | [06 刪掉四層 fallback 梯子](turn-record-fidelity/issues/06-delete-four-source-fallback-ladder.md) | `可交給代理` spec；由本次 `.find()` 取錯 assistant 訊息的實例 bug + `docs/research/deepseek-harness-turn-record-comparison.md` 展開。核心：Host 端一份有 seq/turn/step 的 Turn Record，答案／模型歷史／UI Projection 三者都從它推導（回歸 ADR-0039）。兩個 seam：Host Protocol transcript smoke + 新的 renderer 投影純函式。與 harness-gap-closure 不重疊（spill／smoke.mjs／fork-rerun 仍歸該 effort）。12 張 `可交給代理` tickets：01 可立即開工（止血），02 與 03 隨後；03 之後分成 `04→05→06→07→08` 與 `09` 兩條平行線，10 依賴 05+09，11 依賴 07，12 為完整 qualification |
