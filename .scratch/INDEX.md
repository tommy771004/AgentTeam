...
|--------|------|----------|-------|
| **adaptive-agent-run-status-surface** | [spec.md](adaptive-agent-run-status-surface/spec.md) | [01 自適應 Agent 執行狀態 surface](adaptive-agent-run-status-surface/issues/01-adaptive-run-status-surface.md) | 將預設 rail 分成 trusted「執行狀態」與自適應的任務進度／最近活動／需要你處理／執行摘要；禁止 instructions、reference chat、絕對路徑與 Host revision 混入狀態文案。 |
| **run-review-workspace** | [spec.md](run-review-workspace/spec.md) | [15 Release qualification](run-review-workspace/issues/15-release-qualification.md) | #01–#14 resolved：Review artifact restart recovery、preview-first atomic import、reference-aware retention、archive/hard-delete 與 project rebind 已完成。依票號進 #15 release qualification。 |
| **codex-aligned-personalization-instructions** | [spec.md](codex-aligned-personalization-instructions/spec.md) | [11 External CLI native discovery](codex-aligned-personalization-instructions/issues/11-external-cli-instruction-delivery-modes.md) | 本機 implementation、build、完整 smoke、real Pi Host E2E 與 anti-slop UI gates 已綠；[qualification](codex-aligned-personalization-instructions/qualification.md) 明列 Codex `native_discovery_unproven`、Claude `auth_unavailable`，故維持 needs-info／unqualified，不列 resolved。 |
