...
|--------|------|----------|-------|
| **release-qualification-hardening** | [spec.md](release-qualification-hardening/spec.md) | [06 Custom-tool vault migration](release-qualification-hardening/issues/06-custom-tool-vault-migration.md) | 01–05 已 resolved：release promotion、credential expand contract 與 Telegram/Webhook migration 已有主鏈 owning smoke。Integration runtime、Settings UI、localStorage/settings disk 與 Pi message_send 均改用 main Vault；下一步遷移 custom-tool secrets。Paid Beta 仍維持 NO-GO（0/43）。 |
