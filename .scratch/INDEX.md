...
|--------|------|----------|-------|
| **durable-memory-sqlite-migration** | [spec.md](durable-memory-sqlite-migration/spec.md) | [04 JSON → SQLite 原子遷移與 authority cutover](durable-memory-sqlite-migration/issues/04-atomic-json-sqlite-cutover.md) | #01–03 已 resolved：共用 store contract、SQLite Host protocol、scope/policy/idempotency、validation/quota/sanitizer 與完整 protocol matrix 已掛主 smoke chain；legacy JSON authority 尚未 cutover。frontier 推進 #04，後續為分支遷移 05–13 → failure matrix 14 → contract 15 → qualification 16。 |
