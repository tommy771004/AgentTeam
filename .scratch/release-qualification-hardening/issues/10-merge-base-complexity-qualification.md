...
2026-09-01 — `check-complexity-regression.mts` 現在公開且記錄 baseline policy：CLI argument 與 `COMPLEXITY_BASE_REF` 可覆寫 local 預設 `HEAD^`；GitHub PR path 必須取得 workflow 注入的 `COMPLEXITY_PR_BASE_SHA`，由工具計算 `git merge-base HEAD <base-sha>`，缺值會明確拒絕回落。三提交臨時 repo fixture 在較早 commit 引入 complexity regression、tip 只加無關檔案，證明 merge-base 能抓到而舊 `HEAD^` 行為會漏檢。實際 checkout 相對 PR 對應的 `origin/main` merge-base 無 regression，因此沒有提高 threshold、豁免或跨票重構；spec 建立點包含後續多票已交付差異，不冒充本票 PR baseline。fixture 已納入 `smoke:release`，complexity owner 持續位於 headless `qualify:deterministic`。Focused merge-base／PR fail-closed／release evidence／headless qualification、oxlint 與 production build 全綠；Paid Beta 仍為 NO-GO（0/43）。
