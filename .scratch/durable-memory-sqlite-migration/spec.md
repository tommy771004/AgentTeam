...
**Idempotency.** Runtime writes carry a deterministic operation identity derived from their owning run and tool call or explicit learning source. The store records completed operations under a uniqueness constraint and returns the existing result on retry. Append never uses randomness as its only duplicate defense.
