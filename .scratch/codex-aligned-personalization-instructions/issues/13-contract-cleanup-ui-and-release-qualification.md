...
- [x] Full build、focused instruction smoke、full smoke chain 與 applicable real CLI qualification 綠；未取得的外部真機證據明確列為 blocked，不偽造通過。
- [x] 規格 acceptance、tracker status、DEV_STATE、架構文件與 qualification evidence 完成一 hop 對帳後，才可把 effort 標為 resolved。
- [x] 交付前逐點重讀並執行 repository anti-slop design law 的完整 UI 複查，修正所有發現後留下證據。

**Qualification note（2026-08-30）：** 本票的本機 implementation、release 與 UI gates 已完成，證據集中於 [qualification.md](../qualification.md)。整體 effort 仍不得標為 resolved，因 [#11](11-external-cli-instruction-delivery-modes.md) 的 Codex／Claude native-discovery 真機 acceptance 尚未完成。
