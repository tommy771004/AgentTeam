...
Pi Host resource boundary 是唯一的 instruction discovery owner。Host 的 resolver 先凍結最新版全域指令、include 與專案層級；Pi `DefaultResourceLoader` 的 mutable context-file discovery 明確停用，避免第二次讀檔產生另一份 prompt。Renderer 只編輯 DB-owned 資料、請求受約束的檔案操作並呈現 Host UI Projection，不自行合併可送往模型的第二份 prompt。每次 Task run 由唯一 admission 入口依固定 precedence 產生 immutable instruction snapshot；同一次 run 的後續迭代維持該 snapshot，設定或檔案變更從下一個 run 生效。模型可見的有效文字與來源證據寫入 Turn Record，使 settled、replay、context usage 與稽核使用同一事實來源。
...
56. As a maintainer, I want the Pi Host resource boundary to remain the sole instruction discovery system, so that Pi native context loading or legacy Hermes loading does not return as a second owner.
...
- **Pi Host resource ownership.** Instruction discovery, include expansion and effective snapshot resolution live behind the Pi Host/resource boundary in accordance with the single resource discovery ADR. The Host-created frozen snapshot is injected into Pi while `DefaultResourceLoader.noContextFiles` remains enabled; Pi cannot independently reread mutable AGENTS/CLAUDE files for the same run. Renderer prompt builders stop owning behaviorally equivalent discovery and retain only browser-compatible degradation until its deletion gate is met.
...
- 本 spec 維持 `Status: 待補資訊`：[#11](issues/11-external-cli-instruction-delivery-modes.md) 的 Codex native-discovery 真機證據已於 2026-09-01 通過；Claude 仍為 `auth_unavailable`，因此 provider rollup 與 DEV_STATE 均不得提前標為 resolved。
