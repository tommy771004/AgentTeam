/**
 * Pure tool → permission-key map and policy/projection checks.
 *
 * Leaf module (no store / LLM). Shared by opencode/permissions (FC path) and
 * tools/approvalDecision (authorize path) so the two cannot drift.
 */
import type {
  PermissionAction,
  PermissionKey,
...
export function checkProjectedToolPermission(
  ...
  for (const [pattern, rule] of Object.entries(projection.rules)) {
    ...
    const matchesTool = targets.some(
      (target) => target === key || target === toolName || matchGlob(target, toolName),
    )
    ...
    const value = resolvePatternPermission(
      rule,
      pattern === 'bash' ? command : toolName,
      result || 'allow',
    )
