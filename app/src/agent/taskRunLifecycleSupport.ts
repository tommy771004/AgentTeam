/**
 * Internal task-run lifecycle support.
 *
 * Phase 3 ownership:
 *   item 1 — canonical ingress (`taskRunCoordinator.runTask`)
 ...
 *
 * Store/hook dependencies are loaded lazily; this module never imports the
 * execution owner or public coordinator at runtime.
 */
...
export type FinalizeTaskRunInput = {
  ...
  reenterTask: (input: ExternalRunOpts) => Promise<ExternalRunResult>
  syncExternalSession?: (
    threadId: string,
...
export async function finalizeTaskRun(
  ...
  try {
    await input.syncExternalSession?.(tid, finalAgent.externalRun)
  } catch {
