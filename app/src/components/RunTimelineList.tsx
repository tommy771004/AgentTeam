...
function ReasoningTimelineRow({ row, open, toggle }: { row: Extract<TimelineItem, { kind: 'reasoning' }>; open: boolean; toggle: () => void }) {
  return (
    <div style={{ animation: 'fade-up 320ms cubic-bezier(0.23,1,0.32,1) both' }}>
      <button type="button" aria-expanded={open} data-timeline-row="reasoning" className="agent-process-row flex max-w-full items-center gap-2 text-left text-[12px] text-ink-2" onClick={toggle}>
      ...
      <Reveal open={open}>
        <pre className="agent-process-detail ml-5 mt-0.5 whitespace-pre-wrap text-[11.5px] leading-relaxed text-ink-2 font-[family-name:var(--font-mono)] custom-scrollbar">{row.content}</pre>
      </Reveal>
    </div>
  )
}

function ContextTimelineRow({ row, open, toggle }: { row: Extract<TimelineItem, { kind: 'context' }>; open: boolean; toggle: () => void }) {
  return (
    <div>
      <button type="button" aria-expanded={open} data-timeline-row="context" className="agent-process-row flex max-w-full items-center gap-2 text-left text-[12px] text-ink-2" onClick={toggle}>
...
function ToolTimelineRow({ row, open, toggle }: { row: Extract<TimelineItem, { kind: 'tool' }>; open: boolean; toggle: () => void }) {
  ...
  return (
    <div style={{ animation: 'fade-up 320ms cubic-bezier(0.23,1,0.32,1) both' }}>
      <button type="button" aria-expanded={row.detail ? open : undefined} data-timeline-row="tool" className={`agent-process-row flex max-w-full items-center gap-2 text-left text-[12px] ${failed ? 'text-red' : 'text-ink-2'}`} onClick={() => row.detail && toggle()}>
        <Icon name={pending ? 'progress_activity' : failed ? 'error' : 'terminal'} size={15} className={pending ? 'shrink-0 animate-spin text-ink' : 'shrink-0 text-ink-3'} />
...
function TimelineRowView({ row, open, toggle }: { row: TimelineItem; open: boolean; toggle: () => void }) {
  switch (row.kind) {
    case 'reasoning': return <ReasoningTimelineRow row={row} open={open} toggle={toggle} />
    case 'context': return <ContextTimelineRow row={row} open={open} toggle={toggle} />
    ...
    case 'tool': return <ToolTimelineRow row={row} open={open} toggle={toggle} />
  }
}

export function RunTimelineList({ rows }: { rows: readonly TimelineItem[] }) {
  const [expanded, setExpanded] = useState<string | null>(null)
  return <>{rows.map((row) => <TimelineRowView key={row.id} row={row} open={expanded === row.id} toggle={() => setExpanded((id) => (id === row.id ? null : row.id))} />)}</>
}
