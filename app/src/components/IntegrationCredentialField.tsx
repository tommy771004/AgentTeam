...
import type { CredentialKind, CredentialRef, CredentialVaultMetadata } from '../../electron/credentialVaultAuthority'
import { hydratePluginSecrets } from '../agent/hermes/pluginSecrets'
...
export function IntegrationCredentialField({ kind, ownerId = 'primary', disabled, onChanged }: {
  ...
  const ref: CredentialRef = `credential:${kind}:${ownerId}`
  ...
  return <div className="space-y-2">
    ...
    <input type="password" aria-label={kind === 'custom-tool' ? `新的 ${ownerId} Secret` : kind === 'telegram' ? '新的 Telegram Bot Token' : '新的 Webhook Token'}
