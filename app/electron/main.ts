...
ipcMain.handle('pi-host:turn:submit', async (_evt, input: { sessionId: string; prompt: string; runId?: string; cwd?: string }) => piHostSupervisor.submitTurn(input.sessionId, input.prompt, input.runId, input.cwd))
