...
try {
  ...
  await waitFor(1)
  send(2, 'sessions/create', { title: 'Queue settlement' })
  const sessionId = (await waitFor(2)).result?.sessionId
  assert.ok(sessionId)
  for (const [id, runId] of [[3, 'run-a'], [4, 'run-b']] as const) {
    send(id, 'runs/enqueue', { runId, sessionId, prompt: runId, trigger: 'interactive', profile: {} })
    assert.equal((await waitFor(id)).error, undefined)
  }
  send(5, 'runs/claim')
  const claimed = (await waitFor(5)).result?.run
  assert.equal(claimed?.runId, 'run-a')
  assert.equal(claimed?.status, 'running')
  send(6, 'runs/settle', { runId: 'run-a', settlement: 'answered' })
  assert.equal((await waitFor(6)).result?.queue?.find((item) => item.runId === 'run-a')?.status, 'settled')
  send(7, 'runs/claim', { runId: 'run-b' })
  assert.equal((await waitFor(7)).result?.run?.status, 'running')
} finally {
  host.stdin.end()
