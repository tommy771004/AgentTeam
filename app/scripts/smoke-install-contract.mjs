...
assert.match(runSummaryCard, /reviewSnapshotRef/)
assert.match(runSummaryCard, /查看本次執行的變更/)
assert.match(runSummaryCard, /file\.added/)
assert.match(runSummaryCard, /file\.removed/)
assert.doesNotMatch(runSummaryCard, /UnifiedDiffView/, 'summary cards open the canonical Review Workspace instead of nesting raw patches')
assert.match(installLifecycle, /runPackagedFirstTask/)
assert.match(installLifecycle, /uninstall|detach/)
assert.match(installLifecycle, /message\?\.role === 'assistant'/)
assert.match(installLifecycle, /piHost\?\.sessions/)
assert.match(installLifecycle, /查看本次執行的變更/)
assert.match(installLifecycle, /changePreview/)
