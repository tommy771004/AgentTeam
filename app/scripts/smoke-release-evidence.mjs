...
await test('release workflow requires Windows and macOS packaging evidence', async () => {
  ...
  assert.match(workflow, /- name: Write verification log\n\s+if: always\(\)/)
  assert.match(workflow, /- name: Upload verification logs\n\s+if: always\(\)/)
  assert.match(workflow, /path: app\/release-evidence/)
  assert.match(workflow, /- name: Upload packaged artifacts\n\s+if: success\(\)/)
