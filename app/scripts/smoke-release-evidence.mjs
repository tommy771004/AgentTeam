...
await test('release workflow requires Windows and macOS packaging evidence', async () => {
  ...
  assert.match(packageJob, /npm run qualify:deterministic[\s\S]*npm run build/)
  assert.match(ciWorkflow, /npm run qualify:deterministic[\s\S]*npm run build/)
  assert.match(ciWorkflow, /if: github\.event_name == 'pull_request'[\s\S]*COMPLEXITY_PR_BASE_SHA=\$\{\{ github\.event\.pull_request\.base\.sha \}\}[\s\S]*npm run qualify:deterministic/)
  assert.match(packageJson.scripts['qualify:deterministic'], /npm run smoke:settings-persistence/)
  assert.match(packageJson.scripts['qualify:deterministic'], /npm run smoke:complexity-merge-base/)
