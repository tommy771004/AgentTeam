...
type Session = { id: string; parentSessionId?: string; role?: string; profile?: Record<string, unknown>; context?: { objective: string; facts: string[]; constraints: string[] }; depth?: number; record?: { version: number; entries: Array<{ kind?: string; event?: { state?: string; agentId?: string } }> } }
...
try {
  ...
  assert.deepEqual({ ...childRecord, record: undefined }, {
  ...
  assert.equal(childRecord?.record?.entries[0]?.kind, 'agent-lifecycle')
  assert.deepEqual(childRecord?.record?.entries[0]?.event, {
    agentId: child.result?.sessionId,
    rootAgentId: parentSessionId,
    parentAgentId: parentSessionId,
    taskPath: `/root/analyzer-${String(child.result?.sessionId).slice(-6)}`,
    state: 'admitted',
  })
