...
await test('mcp-apps: allowlisted tool calls cross the approval store and Host MCP pack',()=>{
  ...
  assert.match(surface,/registerMcpAppSurface/)
  assert.match(surface,/surfaceToken: context\.surfaceToken/)
  assert.match(surface,/approvalToken: challenge\.approvalToken/)
  assert.match(surface,/callMcpAppTool/)
  assert.match(surface,/action: 'tool_result'/)
  assert.match(preload,/subdesign:mcpAppToolCall/)
  assert.match(preload,/subdesign:registerMcpAppSurface/)
  assert.match(preload,/subdesign:unregisterMcpAppSurface/)
  assert.match(main,/ipcMain\.handle\('subdesign:mcpAppToolCall'/)
  assert.match(main,/consumeMcpAppSurfaceSession\(event\.sender\.id/)
  assert.match(main,/surface\.allowlist\.has\(coordinate\)/)
  assert.match(main,/issueMcpAppToolApproval/)
  assert.match(main,/takeMcpAppToolApproval/)
  assert.doesNotMatch(main,/const allowlist = Array\.isArray\(input\?\.allowlist\)/)
  assert.doesNotMatch(main,/approval: 'allow'/)
