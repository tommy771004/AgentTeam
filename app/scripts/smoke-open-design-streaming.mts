...
await test('terminal append and unsupported artifact kind are rejected',()=>{
  const terminal=finalizeEnvelope(createStreamingEnvelope('html:done','r'),'complete')
  const late=appendStreamingUpdate(terminal,'late')
  assert.match(late.rejected ?? '',/終止/)
  const unsupported=createStreamingEnvelope('video:clip','r')
  assert.equal(canRender({supportedKinds:['html'],streaming:true,sandbox:"default-src 'none'"},unsupported).ok,false)
})
...
await test('sandboxed HTML cannot access filesystem/network/token',async()=>{
  ...
  const src=fs.readFileSync(path.join(root,'src/agent/subdesign/streamingEnvelope.ts'),'utf8')
...
await test('content visible by default (no entrance animation gate)',async()=>{
  ...
  const anyFile=fs.readFileSync(path.join(root,'src/agent/subdesign/streamingEnvelope.ts'),'utf8')
