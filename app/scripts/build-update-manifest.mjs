...
export async function buildSignedUpdateManifest({ artifactDir, outputDir, version, platform, arch, channel = 'beta', baseUrl, privateKeyPem }) {
  ...
  // Schema-v1 clients released under the old display name reject every other
  // product value. Keep the wire identity stable while UI copy uses AgentStudio.
  const manifest = { schemaVersion: 1, product: 'SubAgents AI', channel, version: String(version).replace(/^v/, ''), platform, arch, mandatory: false, releaseNotes: process.env.UPDATE_RELEASE_NOTES || `AgentStudio ${version}`, publishedAt: new Date().toISOString(), artifact: artifacts[0], signature: '' }
