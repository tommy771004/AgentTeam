...
const root = await mkdtemp(join(tmpdir(), 'pi-read-'))
await writeFile(join(root, 'hello.txt'), 'hello from Pi\nsecond line\n')
const stateDir = await mkdtemp(join(tmpdir(), 'pi-read-state-'))
...
try {
  ...
  assert.equal(response.result?.tool, 'read')
  assert.equal(response.result?.content?.[0]?.text, 'hello from Pi\nsecond line\n')
} finally {
  host.stdin.end()
...
console.log('Pi read tool is exposed through the Host Protocol')
