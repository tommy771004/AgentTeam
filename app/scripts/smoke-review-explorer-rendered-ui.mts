...
try {
  for (const viewport of [{ name: 'desktop', width: 1280, height: 900 }, { name: 'narrow', width: 320, height: 780 }]) {
    ...
    await page.getByRole('button', { name: /載入更多變更（200\/205）/ }).click()
    ...
    assert.equal(await page.locator('button:not([role="option"])').evaluateAll((buttons) => buttons.every((button) => button.querySelector(':scope > .material-symbols-outlined'))), true, `${viewport.name} review actions must remain icon-only`)
