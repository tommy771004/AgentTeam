...
assert.match(explorer, /複製目前 hunk[\s\S]*複製完整 patch/, 'hunk and full-patch copy actions are wired')
assert.match(explorer, /REVIEW_ICON_BUTTON = '[^']*rounded-lg/, 'review action buttons share rounded icon-only styling')
assert.match(explorer, /flex min-h-10[^\n]*gap-0/, 'horizontal review action groups have no spacing')
assert.doesNotMatch(explorer, />(取消|送交核准|折疊 context|Unified|Split|上一 hunk|下一 hunk|複製 hunk|複製 patch)<\/button>/, 'review actions do not render text labels')
