...
try {
  ...
  await waitFor(1)
  send(2, 'tools/bash', { cwd: root, runId: 'bash-cancel-run', callId: 'bash-cancel-a', command: 'printf first; sleep 1; printf never', approval: 'allow' })
  ...
  send(4, 'tools/bash', { cwd: root, runId: 'bash-cancel-run', callId: 'bash-cancel-b', command: 'printf second; sleep 1; printf never', approval: 'allow' })
  ...
  assert.equal(settled.result?.settlement, 'cancelled', JSON.stringify(settled))
  ...
  for (const callId of ['bash-cancel-a', 'bash-cancel-b']) {
    const terminal = messages.filter((item) => item.event === 'host/tool-result' && item.payload?.callId === callId)
    assert.equal(terminal.length, 1, `${callId} settles exactly once`)
    assert.equal(terminal[0].payload?.settlement, 'cancelled')
    assert.ok(!terminal.some((item) => item.payload?.settlement === 'success'), `${callId} has no late success`)
  }
