...
const agentDir = await mkdtemp(join(tmpdir(), 'pi-cancel-agent-'))
const stateDir = await mkdtemp(join(tmpdir(), 'pi-cancel-state-'))
...
await writeFile(join(agentDir, 'models.json'), JSON.stringify({ providers: { loopback: {
  ...
  models: [{ id: 'cancel-model', name: 'Cancel Model', reasoning: false, input: ['text'], contextWindow: 4096, maxTokens: 256 }],
} } }))
await writeFile(join(agentDir, 'auth.json'), JSON.stringify({ loopback: { type: 'api_key', key: 'test-key' } }))
await writeFile(join(agentDir, 'settings.json'), JSON.stringify({ defaultProvider: 'loopback', defaultModel: 'cancel-model', defaultThinkingLevel: 'off' }))
...
try {
  ...
  await waitFor(1)
  send(2, 'sessions/create', { title: 'Cancel smoke' })
  const created = await waitFor(2)
  const sessionId = String(created.result.sessionId)
  send(3, 'turn/submit', { sessionId, runId: 'cancel-run', cwd: process.cwd(), prompt: 'wait forever' })
  await new Promise((resolveDelay) => setTimeout(resolveDelay, 80))
  send(4, 'turn/cancel', { runId: 'cancel-run' })
  const cancelAck = await waitFor(4)
...
console.log('Pi turn cancellation propagates through the Host Protocol')
