...
await test('typed renderer intents expose metadata only and have no raw getter', async () => {
  ...
  for (const relative of [...hostOwners, ...recordOwners]) {
    ...
    assert.doesNotMatch(source, /getVaultSecret|readForUse|credentialVaultAuthority|useIntegrationCredential|integrationCredentialVault/, relative)
  }
...
await test('shipped vault survives fresh module restarts without downgrading remaining credentials', async () => {
  ...
  try {
    ...
    assert.equal(second.useIntegrationCredential('credential:telegram:primary', (secret) => secret), 'telegram-persistent-CANARY')
    ...
    assert.equal(third.useIntegrationCredential('credential:telegram:primary', (secret) => secret), 'rotated-persistent-CANARY')
    ...
    assert.equal(fourth.useIntegrationCredential('credential:telegram:primary', (secret) => secret), 'rotated-persistent-CANARY')
    ...
    assert.throws(() => fifth.useIntegrationCredential('credential:telegram:primary', () => undefined), /not configured/)
    assert.equal(fifth.useIntegrationCredential('credential:webhook:primary', (secret) => secret), 'webhook-persistent-CANARY')
    ...
    assert.equal(locked.useIntegrationCredential('credential:webhook:primary', (secret) => secret), 'webhook-persistent-CANARY')
    ...
    assert.equal(recovered.useIntegrationCredential('credential:webhook:primary', (secret) => secret), 'webhook-persistent-CANARY')
  } finally {
