...
await test('SubDesign Phase 4/5: critique gate + Electron export contract', async () => {
  const fs = await import('node:fs')
  const main = fs.readFileSync(path.join(appRoot, 'electron/main.ts'), 'utf8')
  const workspace = fs.readFileSync(path.join(appRoot, 'src/agent/subdesign/projectWorkspace.ts'), 'utf8')
  const preload = fs.readFileSync(path.join(appRoot, 'electron/preload.ts'), 'utf8')
  ...
  assert.match(workspace, /isProjectRelativePath/)
...
await test('SubDesign Phase 6: canonical metadata, artifact IPC, bash gate, and critique evidence', async () => {
  ...
  assert.match(main, /subdesign:createBrief/)
  assert.match(main, /subdesign:recordOpenDesignPack/)
  assert.doesNotMatch(main, /subdesign:writeMetadata/)
  assert.match(main, /subdesign:registerArtifact/)
  assert.match(workspace, /METADATA_ROOT/)
  assert.match(main, /subdesign:verifyEvidence/)
  assert.match(main, /subdesign:captureEvidence/)
  assert.match(workspace, /captureEvidence/)
  ...
  assert.match(workspace, /verifyEvidence/)
  assert.match(workspace, /attestEvidence/)
  assert.match(workspace, /verifyOne/)
  assert.match(workspace, /lintArtifact/)
  assert.match(workspace, /importReference/)
  assert.match(workspace, /referenceTokens/)
  assert.match(workspace, /selectedDirectionId/)
  assert.match(workspace, /CRITIQUE_SESSION_INCOMPLETE/)
  assert.match(workspace, /readCritiqueSessionAttestation/)
  assert.match(executor, /critiqueSession: theaterSession/)
  assert.match(preload, /readMetadata:/)
  assert.match(preload, /createBrief:/)
  assert.match(preload, /recordOpenDesignPack:/)
  assert.doesNotMatch(preload, /writeMetadata:/)
  assert.match(preload, /registerArtifact:/)
...
await test('Open Design Phase 0/1/2: inventory, safe pack boundary, project copy, and provenance wiring', async () => {
  ...
  assert.match(workspace, /Open Design pack 超過複製大小限制/)
  assert.match(workspace, /design-system pack 必須包含 DESIGN\.md/)
