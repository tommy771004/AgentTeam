...
await test('SubDesign R3-P4/P5: critique theater and Design System resource routes', async () => {
  ...
  // The browse-and-apply affordance moved from inline copy to the studio nav:
  // SubDesign links out to /design-systems carrying returnTo + briefId, and the
  // chosen system is written back onto the brief (asserted just below).
  assert.match(subDesignPage, /onBrowseDesignSystems|onOpenDesignSystems/)
  assert.match(subDesignPage, /\/design-systems\?returnTo=/)
  assert.match(subDesignPage, /briefId=\$\{encodeURIComponent/)
