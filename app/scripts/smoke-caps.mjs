...
await test('permissionAskStore tracks timedOut + runStats for archive', async () => {
  ...
  const panel = fs.readFileSync(path.join(appRoot, 'src/components/PermissionAskModal.tsx'), 'utf8')
  const presentation = fs.readFileSync(path.join(appRoot, 'src/components/DecisionCard.tsx'), 'utf8')
  const protocols = fs.readFileSync(path.join(appRoot, 'src/pages/ProtocolsPage.tsx'), 'utf8')
  assert.match(panel, /current\.runId/)
  assert.match(panel, /resolve\(current\.id, 'allow'/)
  assert.match(panel, /resolve\(current\.id, 'deny'\)/)
  assert.doesNotMatch(panel, /remainSec|setInterval|自動拒絕/)
  assert.doesNotMatch(presentation, /<dialog|showModal|aria-modal/)
  assert.match(presentation, /role="region"/)
  assert.match(protocols, /<PermissionAskPanel threadId=\{activeId\}/)
})
